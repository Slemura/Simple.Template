...
public class StylizedSurfaceEditor : ShaderGUI
{
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        var originalIntentLevel = EditorGUI.indentLevel;
        var foldoutRemainingItems = 0;
        var latestFoldoutState = false;
        ...
        foreach (var property in properties)
        {
            ...
            {
                var foldoutName = displayName.Split('(', ')')[1];
                var foldoutItemCount = displayName.Split('{', '}')[1];
                foldoutRemainingItems = Convert.ToInt32(foldoutItemCount);
                if (!_foldoutStates.ContainsKey(property.name)) _foldoutStates.Add(property.name, false);
            ...
            if (_target.IsKeywordEnabled("_CELPRIMARYMODE_STEPS") && displayName.Contains("[LAST_PROP_STEPS]"))
            {
                EditorGUILayout.HelpBox(
                    ...
                    MessageType.Info);
                var currentNumSteps = _target.GetInt("_CelNumSteps");
            ...
            var hideInInspector = (property.flags & MaterialProperty.PropFlags.HideInInspector) != 0;
            if (!hideInInspector && !skipProperty) DrawStandard(property);

            EditorGUI.indentLevel = originalIntentLevel;
        }

        if (UnityEngine.Rendering.GraphicsSettings.renderPipelineAsset != null) HandleUrpSettings(_target, _editor);
    ...
    {
        var alphaClip = false;
        if (material.HasProperty("_AlphaClip")) alphaClip = material.GetFloat("_AlphaClip") >= 0.5;
    ...
    {
        var numSteps = _celShadingNumSteps;
    ...
    private Texture2D GenerateCurveTexture()
    {
        ...
        {
            var value = _gradient.Evaluate(x / width);
    ...
    private void SaveTextureAsPng(Texture2D texture, string fullPath, FilterMode filterMode)
    {
        ...
        var pathRelativeToAssets = ConvertFullPathToAssetPath(fullPath);
        var importer = (TextureImporter)AssetImporter.GetAtPath(pathRelativeToAssets);
        if (importer != null)
        {
